...
export function CounterCreate() {
  ...
  return (
   <div>
    ...
    className='input input-bordered w-full max-w-rs' />
...
function CounterCard({ account }: { account: PublicKey }) {
  ...
  return accountQuery.isLoading ? (
    <span className='loading loading-spinner loading-lg'></span> ) : (
    <div className="card card-bordered bg-base-300 border-4 text-neutral-content">
      <div className="card-body justify-between items-center text-center">
        ...
        value={message}
        onChange={(e)=>{setMessage(e.target.value)}}
        className='textarea textarea-bordered w-full max-w-rs'/>
          <button 
          onClick={handleSubmit}
